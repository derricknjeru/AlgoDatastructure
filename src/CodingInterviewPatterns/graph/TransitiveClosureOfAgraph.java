...
Given a directed CodingInterviewPatterns.graph, return its connectivity matrix. The value of a cell C[i][j] in connectivity matrix C is 1 only if a directed path exists from vertex i to vertex j.
...
Note.md.md.md.md that all diagonal elements in the connectivity matrix are 1 since a path exists from every vertex to itself.

Constraints:

• The CodingInterviewPatterns.graph is implemented using an adjacency list.
• The maximum number of nodes in the CodingInterviewPatterns.graph is 100, i.e., 0 <= n < 100, and each node is represented by its numeric value.
...
*/
    ...
    class Graph {
        ...
        // Total number of nodes in the CodingInterviewPatterns.graph
        int n;
    }
